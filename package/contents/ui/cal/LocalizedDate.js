...
function formatEventTime(dateTime, args) {
	...
	var hourlyShortForm = args && typeof args.hourlyShortForm !== "undefined" ? args.hourlyShortForm : true
	var timeFormat
	if (clock24h) {
		if (hourlyShortForm && dateTime.getMinutes() == 0) {
...
function formatEventDuration(event, args) {
	...
	if (event.start.date) {
		...
		// console.log('formatEventDuration is all day', event.summary)
		if (TimeUtils.isSameDate(startTime, dayBefore)) {
			return i18n("All Day")
		} else {
		...
		// console.log('formatEventDuration not all day', event.summary)
		var startStr
		if (!relativeDate || !TimeUtils.isSameDate(startTime, relativeDate)) {
		...
		var endStr
		if (TimeUtils.isSameDate(startTime, endTime)) {
